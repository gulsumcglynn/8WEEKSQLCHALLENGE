...
SELECT COUNT(DISTINCT customer_id) AS musteri_sayısı
FROM subscriptions;
...
SELECT SUM(CASE WHEN p.plan_name = 'churn' THEN 1 ELSE 0 END) sum_churn,
       CAST(100*SUM(CASE WHEN p.plan_name = 'churn' THEN 1 ELSE 0 END) AS FLOAT(1))/COUNT(DISTINCT s.customer_id) as churn_percentage
FROM subscriptions s
JOIN plans p on p.plan_id = s.plan_id;

--5.How many customers have churned straight after their initial free trial - what percentage is this rounded to the nearest whole number?
WITH PlanDegisiklikleri AS (
    SELECT 
        s.customer_id,
        p.plan_name,
        s.start_date,
        LEAD(p.plan_name, 1) OVER (PARTITION BY s.customer_id ORDER BY s.plan_id) AS SonrakiPlan
    FROM subscriptions s
	JOIN plans p on s.plan_id = p.plan_id
)
SELECT 
...
WHERE 
    plan_name = 'trial'
    AND SonrakiPlan = 'churn'
GROUP BY customer_id,plan_name,SonrakiPlan;

--Toplam 92 churn var %9 a tekabul ediyor.
...
SELECT 
    SonrakiPlan AS Plan_adı, 
    COUNT(*) AS ToplamGecis,
	100*COUNT(*) / (SELECT COUNT(DISTINCT customer_id) FROM subscriptions) AS churn_percentage
FROM PlanDegisiklikleri
WHERE 
    plan_name = 'trial'
    AND SonrakiPlan IS NOT NULL
GROUP BY SonrakiPlan;
...
WITH PlanDegisiklikleri AS (
    SELECT 
        ...
        s.start_date,
		s.plan_id,
        LEAD(start_date, 1) OVER (PARTITION BY s.customer_id ORDER BY s.start_date) AS SonrakiPlan
    FROM subscriptions s
	JOIN plans p on s.plan_id = p.plan_id
)
SELECT plan_id,
...
FROM PlanDegisiklikleri
WHERE (SonrakiPlan IS NOT NULL AND (start_date < '2020-12-31' AND SonrakiPlan > '2020-12-31'))
  OR (SonrakiPlan IS NULL AND start_date < '2020-12-31')
GROUP BY plan_name,plan_id;

--8.How many customers have upgraded to an annual plan in 2020?
SELECT 
       COUNT(DISTINCT customer_id) musteri_sayısı,
	   plan_name
FROM subscriptions s
JOIN plans p on p.plan_id = s.plan_id
...
WITH ANNUAL AS(
             SELECT start_date AS annual_start_date,
			        s.customer_id,
					plan_name
             FROM subscriptions s 
             JOIN plans p on p.plan_id = s.plan_id
     ...
     TRIAL AS(  
             SELECT start_date AS trial_start_date,
			        s.customer_id,
					plan_name
             FROM subscriptions s 
             JOIN plans p on p.plan_id = s.plan_id
...
SELECT AVG(DATEDIFF(DAY, trial_start_date, annual_start_date)) AS avg_start_date
FROM ANNUAL a
JOIN TRIAL t on a.customer_id = t.customer_id;
--104 
...
SELECT COUNT(*) sum_customer
FROM PlanDegisiklikleri
WHERE plan_name = 'pro monthly'
      AND SonrakiPlan = 'basic monthly'
